...
static char remote_device_name [ ESP_BLE_ADV_NAME_LEN_MAX ] =
    "ESP_BLE_SECURITY";
...
static void gattc_profile_event_handler( esp_gattc_cb_event_t      event,
    ...
    case ESP_GATTC_REG_EVT :
        ESP_LOGI( GATTC_TAG,
                  "GATT client register, status %u, app_id %u, gattc_if %d",
                  p_data->reg.status,
                  p_data->reg.app_id,
                  gattc_if );
    ...
    case ESP_GATTC_OPEN_EVT :
        if ( param->open.status != ESP_GATT_OK ) {
            ESP_LOGE(
                GATTC_TAG, "Open failed, status %x", p_data->open.status );
            break;
        }
    ...
    case ESP_GATTC_CFG_MTU_EVT :
        ESP_LOGI( GATTC_TAG,
                  "MTU exchange, status %d, MTU %d",
                  param->cfg_mtu.status,
                  param->cfg_mtu.mtu );
        esp_ble_gattc_search_service(
            gattc_if, param->cfg_mtu.conn_id, &remote_filter_service_uuid );
        break;
    case ESP_GATTC_SEARCH_RES_EVT : {
        ESP_LOGI( GATTC_TAG,
                  "Service search result, conn_id %x, is primary service %d",
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT :
        if ( p_data->search_cmpl.status != ESP_GATT_OK ) {
            ESP_LOGE( GATTC_TAG,
                      "Service search failed, status %x",
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT : {
        if ( p_data->reg_for_notify.status != ESP_GATT_OK ) {
            ESP_LOGE( GATTC_TAG,
                      "Notification register failed, status %x",
        ...
        if ( ret_status != ESP_GATT_OK ) {
            ESP_LOGE(
                GATTC_TAG, "esp_ble_gattc_get_attr_count error, %d", __LINE__ );
            break;
        }
    ...
    case ESP_GATTC_NOTIFY_EVT :
        ...
        ESP_LOG_BUFFER_HEX(
            GATTC_TAG, p_data->notify.value, p_data->notify.value_len );
        break;
    case ESP_GATTC_WRITE_DESCR_EVT :
        if ( p_data->write.status != ESP_GATT_OK ) {
            ESP_LOGE( GATTC_TAG,
                      "Descriptor write failed, status %x",
                      p_data->write.status );
            break;
        }
    ...
    case ESP_GATTC_SRVC_CHG_EVT : {
        ...
        ESP_LOGI( GATTC_TAG,
                  "Service change from " ESP_BD_ADDR_STR "",
                  ESP_BD_ADDR_HEX( bda ) );
        break;
    }
    case ESP_GATTC_WRITE_CHAR_EVT :
        if ( p_data->write.status != ESP_GATT_OK ) {
            ESP_LOGE( GATTC_TAG,
                      "Characteristic write failed, status %x",
                      p_data->write.status );
            break;
        }
...
static void esp_gap_cb( esp_gap_ble_cb_event_t  event,
    ...
    case ESP_GAP_BLE_SET_LOCAL_PRIVACY_COMPLETE_EVT :
        if ( param->local_privacy_cmpl.status != ESP_BT_STATUS_SUCCESS ) {
            ESP_LOGE( GATTC_TAG,
                      "Local privacy config failed, status %x",
        ...
        if ( scan_ret ) {
            ESP_LOGE(
                GATTC_TAG, "set scan params error, error code = %x", scan_ret );
        }
        break;
    ...
    case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT :
        ...
        if ( param->scan_start_cmpl.status != ESP_BT_STATUS_SUCCESS ) {
            ESP_LOGE( GATTC_TAG,
                      "Scanning start failed, status %x",
    ...
    case ESP_GAP_BLE_OOB_REQ_EVT : {
        ...
        esp_ble_oob_req_reply(
            param->ble_security.ble_req.bd_addr, tk, sizeof( tk ) );
        break;
    }
    ...
    case ESP_GAP_BLE_NC_REQ_EVT :
        ...
        ESP_LOGI( GATTC_TAG,
                  "Numeric Comparison request, passkey %" PRIu32,
    ...
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT : /// the app will receive this evt when
        ...
        ESP_LOGI( GATTC_TAG,
                  "Passkey notify, passkey %06" PRIu32,
                  param->ble_security.key_notif.passkey );
        break;
    case ESP_GAP_BLE_KEY_EVT :
        // shows the ble key info share with peer device to the user.
        ESP_LOGI( GATTC_TAG,
                  "Key exchanged, key_type %s",
    ...
    case ESP_GAP_BLE_AUTH_CMPL_EVT : {
        ...
        memcpy( bd_addr,
                ...
                sizeof( esp_bd_addr_t ) );
        ESP_LOGI( GATTC_TAG,
                  "Authentication complete, addr_type %d, addr " ESP_BD_ADDR_STR
                  "",
                  param->ble_security.auth_cmpl.addr_type,
                  ESP_BD_ADDR_HEX( bd_addr ) );
        if ( ! param->ble_security.auth_cmpl.success ) {
            ESP_LOGI( GATTC_TAG,
                      "Pairing failed, reason 0x%x",
                      param->ble_security.auth_cmpl.fail_reason );
        } else {
            ESP_LOGI( GATTC_TAG,
                      "Pairing successfully, auth mode %s",
                      esp_auth_req_to_str(
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT :
            ...
            if ( adv_name != NULL ) {
                if ( strlen( remote_device_name ) == adv_name_len &&
                    ...
                    // it is unique.
                    ESP_LOGI(
                        GATTC_TAG, "Device found %s", remote_device_name );
                    if ( connect == false ) {
                        connect = true;
...
static void esp_gattc_cb( esp_gattc_cb_event_t      event,
                          esp_gatt_if_t             gattc_if,
                          esp_ble_gattc_cb_param_t* param ) {
    ESP_LOGI( GATTC_TAG, "esp_gattc_cb EVT %d, gattc if %d", event, gattc_if );
...
void ble_client_init( void ) {
    ...
    memcpy( remote_device_name,
            esp_bluedroid_get_example_name( ),
            sizeof( remote_device_name ) );
#endif
    ...
    if ( ret ) {
        ESP_LOGE( GATTC_TAG,
                  "%s gap register error, error code = %x",
                  __func__,
                  ret );
        return;
    }
    ...
    if ( ret ) {
        ESP_LOGE( GATTC_TAG,
                  "%s gattc register error, error code = %x",
                  __func__,
                  ret );
        return;
    }
    ...
    if ( ret ) {
        ESP_LOGE( GATTC_TAG,
                  "%s gattc app register error, error code = %x",
                  __func__,
                  ret );
    }
