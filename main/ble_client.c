...
#define GATTC_PROFILE_HID (gl_profile_tab[PROFILE_HID_APP_ID])
...
static bool is_get_service = false;
bool is_send_info = false;
static char device_name_base[ESP_BLE_ADV_NAME_LEN_MAX - 4] = "ESP32_HIDBridge";

char device_name[ESP_BLE_ADV_NAME_LEN_MAX] = {0};
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
    ...
    switch (event) {
        ...
        case ESP_GATTC_CONNECT_EVT:
            ESP_LOGD(GATTC_TAG, "Connected, conn_id %d, remote " ESP_BD_ADDR_STR "", p_data->connect.conn_id,
        ...
        case ESP_GATTC_OPEN_EVT:
            ...
            ESP_LOGD(GATTC_TAG, "Open successfully, MTU %d", p_data->open.mtu);
            GATTC_PROFILE_HID.conn_id = p_data->open.conn_id;
            memcpy(GATTC_PROFILE_HID.remote_bda, p_data->open.remote_bda, sizeof(esp_bd_addr_t));
        ...
        case ESP_GATTC_CFG_MTU_EVT:
            ESP_LOGD(GATTC_TAG, "MTU exchange, status %d, MTU %d", param->cfg_mtu.status, param->cfg_mtu.mtu);
            esp_ble_gattc_search_service(gattc_if, param->cfg_mtu.conn_id, &remote_filter_service_uuid);
            break;
        case ESP_GATTC_SEARCH_RES_EVT: {
            ESP_LOGD(GATTC_TAG, "Service search result, conn_id %x, is primary service %d", p_data->search_res.conn_id,
                     p_data->search_res.is_primary);
            ESP_LOGD(GATTC_TAG, "start handle %d end handle %d current handle value %d",
            ...
            if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 &&
                ...
                is_get_service = true;
                GATTC_PROFILE_HID.service_start_handle = p_data->search_res.start_handle;
                GATTC_PROFILE_HID.service_end_handle = p_data->search_res.end_handle;
            }
            break;
        }
        case ESP_GATTC_SEARCH_CMPL_EVT:
            ...
            ESP_LOGD(GATTC_TAG, "Service search complete");
            if (is_get_service) {
                uint16_t count = 0;
                uint16_t offset = 0;
                esp_gatt_status_t ret_status =
                    esp_ble_gattc_get_attr_count(gattc_if, GATTC_PROFILE_HID.conn_id, ESP_GATT_DB_CHARACTERISTIC,
                                                 GATTC_PROFILE_HID.service_start_handle,
                                                 GATTC_PROFILE_HID.service_end_handle, INVALID_HANDLE, &count);
                ...
                if (count > 0) {
                    ...
                    if (!char_elem_result) {
                        ...
                        ret_status = esp_ble_gattc_get_all_char(
                            gattc_if, GATTC_PROFILE_HID.conn_id, GATTC_PROFILE_HID.service_start_handle,
                            GATTC_PROFILE_HID.service_end_handle, char_elem_result, &count, offset);
                        if (ret_status != ESP_GATT_OK) {
                        ...
                        if (count > 0) {
                            for (int i = 0; i < count; ++i) {
                                if (char_elem_result[i].uuid.len == ESP_UUID_LEN_16 &&
                                    char_elem_result[i].uuid.uuid.uuid16 == KEYWORD_CHAR_UUID &&
                                    (char_elem_result[i].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY)) {
                                    GATTC_PROFILE_HID.keyword_char_handle = char_elem_result[i].char_handle;
                                    esp_ble_gattc_register_for_notify(gattc_if, GATTC_PROFILE_HID.remote_bda,
                                           ...
                                           char_elem_result[i].uuid.uuid.uuid16 == SENSOR_CHAR_UUID &&
                                           (char_elem_result[i].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY)) {
        ...
        case ESP_GATTC_WRITE_DESCR_EVT:
            if (p_data->write.status != ESP_GATT_OK) {
                ESP_LOGE(GATTC_TAG, "Descriptor write failed, status %02X", p_data->write.status);
                break;
            }
            ESP_LOGD(GATTC_TAG, "Descriptor write successfully");
            break;
        ...
        case ESP_GATTC_WRITE_CHAR_EVT:
            ...
            ESP_LOGD(GATTC_TAG, "Characteristic write successfully");
            break;
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t* param) {
    ...
    switch (event) {
        case ESP_GAP_BLE_SET_LOCAL_PRIVACY_COMPLETE_EVT:
            ...
            ESP_LOGD(GATTC_TAG, "Local privacy config successfully");
        ...
        case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
            ...
            ESP_LOGD(GATTC_TAG, "Scanning start successfully");
            break;
        case ESP_GAP_BLE_PASSKEY_REQ_EVT: /* passkey request event */
            /* Call the following function to input the passkey which is displayed
             * on the remote device */
            // esp_ble_passkey_reply(GATTC_PROFILE_HID.remote_bda,
            // true, 0x00);
            ESP_LOGD(GATTC_TAG, "Passkey request");
            break;
        case ESP_GAP_BLE_OOB_REQ_EVT: {
            ESP_LOGD(GATTC_TAG, "OOB request");
            uint8_t tk[16] = {1};  // If you paired with OOB, both devices need to use the same tk
            esp_ble_oob_req_reply(param->ble_security.ble_req.bd_addr, tk, sizeof(tk));
            break;
        }
        case ESP_GAP_BLE_LOCAL_IR_EVT: /* BLE local IR event */
            ESP_LOGD(GATTC_TAG, "Local identity root");
            break;
        case ESP_GAP_BLE_LOCAL_ER_EVT: /* BLE local ER event */
            ESP_LOGD(GATTC_TAG, "Local encryption root");
            break;
        ...
        case ESP_GAP_BLE_NC_REQ_EVT:
            ...
            ESP_LOGD(GATTC_TAG, "Numeric Comparison request, passkey %" PRIu32, param->ble_security.key_notif.passkey);
            break;
        case ESP_GAP_BLE_PASSKEY_NOTIF_EVT:  /// the app will receive this evt when
            ...
            ESP_LOGD(GATTC_TAG, "Passkey notify, passkey %06" PRIu32, param->ble_security.key_notif.passkey);
            break;
        case ESP_GAP_BLE_KEY_EVT:
            // shows the ble key info share with peer device to the user.
            ESP_LOGD(GATTC_TAG, "Key exchanged, key_type %s",
                     esp_key_type_to_str(param->ble_security.ble_key.key_type));
            break;
        case ESP_GAP_BLE_AUTH_CMPL_EVT: {
            esp_bd_addr_t bd_addr;
            memcpy(bd_addr, param->ble_security.auth_cmpl.bd_addr, sizeof(esp_bd_addr_t));
            ESP_LOGD(GATTC_TAG, "Authentication complete, addr_type %d, addr " ESP_BD_ADDR_STR "",
                     param->ble_security.auth_cmpl.addr_type, ESP_BD_ADDR_HEX(bd_addr));
            if (!param->ble_security.auth_cmpl.success) {
                ESP_LOGD(GATTC_TAG, "Pairing failed, reason 0x%x", param->ble_security.auth_cmpl.fail_reason);
            } else {
                ESP_LOGD(GATTC_TAG, "Pairing successfully, auth mode %s",
        ...
        case ESP_GAP_BLE_SCAN_RESULT_EVT: {
            ...
            switch (scan_result->scan_rst.search_evt) {
                case ESP_GAP_SEARCH_INQ_RES_EVT:
                    ...
                    ESP_LOGD(GATTC_TAG, "Scan result, device " ESP_BD_ADDR_STR ", name len %u",
                    ...
                    if (adv_name != NULL) {
                        if (strlen(device_name) == adv_name_len &&
                            ...
                            // Note: If there are multiple devices with the same device
                            // name, the device may connect to an unintended one. It is
                            // recommended to change the default device name to ensure
                            // it is unique.
                            ESP_LOGD(GATTC_TAG, "Device found %s", device_name);
                            if (is_connect == false) {
                                is_connect = true;
                                ...
                                esp_ble_gattc_enh_open(GATTC_PROFILE_HID.gattc_if, &creat_conn_params);
                            }
                        }
                    }
                    break;
...
static void esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t* param) {
    ESP_LOGD(GATTC_TAG, "esp_gattc_cb EVT %d, gattc if %d", event, gattc_if);
    ...
    if (event == ESP_GATTC_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ...
            ESP_LOGD(GATTC_TAG, "Reg app failed, app_id %04x, status %d", param->reg.app_id, param->reg.status);
            return;
        }
    }
...
void ble_client_init(void) {
    ...
    snprintf(device_name, sizeof(device_name), "%s_%02x", device_name_base, 0);
    ...
    ret = esp_ble_gattc_app_register(PROFILE_HID_APP_ID);
    if (ret) {
